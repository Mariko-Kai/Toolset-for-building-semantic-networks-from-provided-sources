...
\section{Логические связки (Logical Connectives)}\label{entity:def-logical-connectives}
\textbf{Тип:} object\quad \textbf{Источник:} Зорич В.А., Математический анализ, Том I (zorich-1, p. 2)
...
\textbf{Естественный язык:}
Базовые операции математической логики (пропозициональные связки), используемые для построения сложных высказываний:
\begin{itemize}
    \item \textbf{Конъюнкция} ($\land$): Логическое И. Истинно, только если оба операнда истинны.
    \item \textbf{Дизъюнкция} ($\lor$): Логическое ИЛИ. Истинно, если хотя бы один операнд истинен.
    \item \textbf{Отрицание} ($\lnot$): Логическое НЕ. Меняет истинность операнда на противоположную.
    \item \textbf{Импликация} ($\implies$): Логическое следствие (ЕСЛИ..., ТО...). Ложно только в случае, когда из истины следует ложь.
    \item \textbf{Эквивалентность} ($\iff$): Логическое равенство (ТОГДА И ТОЛЬКО ТОГДА). Истинно, если оба операнда имеют одинаковую истинность.
\end{itemize}
