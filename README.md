...
`lean_validator/` с зависимостями Mathlib и `repl`. Таймаут одной проверки задаётся
переменной `MATHESIS_LEAN_TIMEOUT` (по умолчанию 300 с; первый запуск дольше из-за загрузки
Mathlib).
...
| `MATHESIS_LEAN_TIMEOUT` | Таймаут одной проверки Lean, секунды (по умолчанию 300) |
