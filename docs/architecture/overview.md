...
1. **PDF → Изображения:** утилита `pipeline/pdftoimages/pdf_to_images.py` конвертирует страницы учебника в изображения для точного распознавания формул.
2. **Изображения → Stub .tex:** агент (`pipeline/agent/agent.py`) на базе Gemini API анализирует изображения страниц, выделяет сущности и создаёт файлы-заглушки.
...
4. **LaTeX → SQLite:** скрипт `pipeline/link_content.py` парсит `.tex` файлы и заполняет базу `mathesis_index.db`.
