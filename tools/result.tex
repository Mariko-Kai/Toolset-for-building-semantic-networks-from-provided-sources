...
\[
\mLim_{n \to \infty} x_n = L \mDefIff \Forall{\varepsilon > 0}\; \mExists{n_0 \in \mathbb{N}}\; \Forall{n \in \mathbb{N}} \colon \left( n \geq n_0 \mImplies \entityref{op-abs-abstract}{\mathrm{abs}}(x_n - L) < \varepsilon \right)
\]
\end{operation}
...
\[
\Forall{A, B}
(A = B) \mIff (\Forall{x} x \in A \mIff x \in B)
\]
...
\[
\Forall{\mathcal{F}}
\mExists{U} \Forall{x} \left( x \in U \mIff \mExists{A} (A \in \mathcal{F} \land x \in A) \right)
\]
...
\[
\Forall{A, B}
\mExists{C} \Forall{x} \left( x \in C \mIff (x = A \entityref{term-disjunction}{\lor} x = B) \right)
\]
